...
> 轻量 RAG 引擎 + LLM-Wiki 渐进式知识库。16+ 端点，2 个小模型，API-only。
...
4. **纯 API，不内置 LLM** — nollmkb 本身不调任何大模型。检索结果喂给 ChatGPT、Claude、OpenCode 等外部 LLM，由它们生成答案或写 wiki
...
nollmkb 本身没有 UI，所有的交互通过 LLM agent（OpenCode、Claude Code、Codex 等）完成。你把 `API.md` 丢给 agent，它就能自己调接口。
...
| Reranker | BGE-Reranker-v2-m3 (懒加载) | ~4.0 GB VRAM |
...
|---|---|---|---|
| API | 6 (RAG) + 10 (Wiki) | 通用向量 API | 完整但重 |
| 去重 | 三级 | 无 | 无 |
| Reranker | ✅ | ❌ | 含 LLM |
| Context | ✅ | ❌ | ❌ |
| 千万 token | ✅ 未测极限 | ✅ | ✅ |
